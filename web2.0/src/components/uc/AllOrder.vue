<template>
	<div class="rOrder">
		<div class="orderHead" style="text-indent:30px;    font-size: 16px;">
			<strong>所有订单</strong>
		</div>
			...
			<div class="contentShow">
				...
				<div class="showBody" v-for="(item,index) in orderInfo" :key="index">
					<ul class="bodyHead ">
                        ...
                        <li>订单类型 : <span style="color:#999" v-if="item.orderCategory==0">pc 订单</span>
                        <span style="color:#999"  v-if="item.orderCategory==1">微信 订单</span></li>
                       <span style="color:#999"  v-if="item.orderCategory==1">安卓app 订单</span>
                           <span style="color:#999"  v-if="item.orderCategory==1">iso app订单</span>
					...
					<ul class="bodyContent orderlist">
						...
						<li>
							...
							<Button type="primary"  v-else-if="item.orderStatus == '2'"
									@click="operateBtn(item.orderNo,item.orderStatus)">确认收货
							</Button>
							<Button type="primary"  v-else-if="item.orderStatus == '3'"
							...
							<Button type="error"  v-else-if="item.orderStatus == '5'"
									@click="deleteBtn(item.orderNo)">删除订单
							</Button>
							<Button type="error"  v-else @click="deleteBtn(item.orderNo)">删除订单</Button>
						</li>
					</ul>
				</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			info() {
				this.$http.post('/order/orderList',qs.stringify({userType:'0',orderCategory:'0'})).then(this.orderListSucc);
			}
		},
		mounted() {
			this.info()
